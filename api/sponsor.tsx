...
export default async function handler(request: Request): Promise<any> {
  try {
    ...
    const id = searchParams.get('id') || 'arietta-studio';
    const data = await fetchOpenCollectiveData(id);
    const { fontBold, fontRegular } = await fetchFonts();
    if (avatarSize === undefined) return new Response('avatarSize is required', { status: 400 });
    if (width === undefined) return new Response('width is required', { status: 400 });
